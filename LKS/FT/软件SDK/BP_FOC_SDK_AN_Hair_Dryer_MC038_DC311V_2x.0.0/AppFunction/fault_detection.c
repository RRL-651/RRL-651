...
stru_FaultTime_t struFaultZeroTime;
/*****************************************************************************
...
void FaultStartCheck(s32 wSpeedFbk, s16 nIq, stru_CurrPhaseUVW *pstruIabcMax, stru_FaultTime_t *pstruFaultTime)
{
    if(struFOC_CtrProc.eSysState == RUN)
    {
        ...
        {
            if(pstruFaultTime->nCheckCnt1 < STALL_CHCEK_TIME)  //滤波时间
            {
                pstruFaultTime->nCheckCnt1 ++;
            }
            else                                            //判定为启动错误
        ...
        {
            if(pstruFaultTime->nCheckCnt2 < STALL_CHCEK_TIME)      //滤波时间
            {
                pstruFaultTime->nCheckCnt2 ++;
            }
            else                                 //判定为启动错误
 ...
 * 返回值   ：无
 * 修改时间 ：2020.08.17   2023.7.5
 ***********************************************************************************/
u8 NTCERRORTime_min=0;
u8 TempOverErrorstate;
void FaultTempCheck(s16 nTemperature, stru_FaultTime_t *pstruFaultTime)
{
    ...
    else
    {
		...
		if((nTemperature > FRSControl.TargetTemperature + 1000))    //当工作加热过程中，NTC在未达到设定温度前，连续检测超过5分钟，NTC温度不变化或者变化不超过2°
    {
        pstruFaultTime->nCheckCnt2 ++;
			if(pstruFaultTime->nCheckCnt2>=12000)
			{
				NTCERRORTime_min++;
				pstruFaultTime->nCheckCnt2 = 0;
			}
    ...
    if(pstruFaultTime->nCheckCnt1 > 500)//滤波时间2s，400*5ms = 2000ms
    {
        PWMOutputs(MCPWM0, DISABLE);
        stru_Faults.B.TempOverError = 1;//温度故障标志位置1
        pstruFaultTime->nCheckCnt1 = 0;
			if(nTemperature < TEMP_FAULT)TempOverErrorstate=0;
			else TempOverErrorstate=1;
    }
		if(NTCERRORTime_min >= 5)//滤波时间2s，400*5ms = 2000ms
    {
        PWMOutputs(MCPWM0, DISABLE);
        stru_Faults.B.NTCError = 1;//温度故障标志位置1
        pstruFaultTime->nCheckCnt2 = 0;
				NTCERRORTime_min=0;
    }
}
...
/***********************************************************************************
 * 函数名   : FaultZeroCheck(s16 ZEROms, stru_FaultTime_t *pFaultTime)
 * 说明     : 当过零信号翻转周期不在50hz和60hz的频率范围内，认为为过零异常；
 * 设计思路 ：1.根据过零检测时间去判断是否发生过零异常故障
 * 参数     ：s16 ZEROms, stru_FaultTime_t *pFaultTime
 * 返回值   ：无
 * 修改时间 ：2023.07.15
 ***********************************************************************************/
void FaultZeroCheck(s16 ZEROms, stru_FaultTime_t *pstruFaultTime)
{
    if((ZEROms < ZEROUNDER_FAULT) || (ZEROms > ZEROOVER_FAULT))    //当过零信号翻转周期不在50hz和60hz的频率范围内，认为为过零异常；
    {
        pstruFaultTime->nCheckCnt1 ++;
    }
    else
    ...
    {
        PWMOutputs(MCPWM0, DISABLE);
        stru_Faults.B.ZeroError = 1;//温度故障标志位置1
        pstruFaultTime->nCheckCnt1 = 0;
    }
}
/***********************************************************************************
...
//						if((FRSControl.TurnOnOffFlag == 1)&&(TempGear !=0))   //发热丝正常工作
//						{
//							FaultTempCheck(FRSControl.HEATER_NTCTemperature, &struFaultTempTime);						//过温检测
//						}
//						FaultZeroCheck(FRSControl.CrossZeroMsCnt, &struFaultZeroTime);									//过零异常检测
            break;
        }
        case 4:
        {
