...
#define RSHUNT                         (0.100)             /* 单位: Ω  采样电阻阻值 */
#define VOLTAGE_SHUNT_RATIO            (1.0/(202.1)) /* 母线电压分压比 */
...
#define U_MOTOR_LQ                     (2900.4)           //单位: uH 电机q轴电感
...
#define PowerUP_Motor_Start_Dir        (CCW)                                         
...
/* ----------------------------预驱自举电容预充电参数--------------------------- */
#define CHARGE_TIME                    (50)              /* 每相预充电时间，根据实际硬件参数修改 */
...
#define U_START_CUR_SET_S              (0.65)              /* 单位: A 第二段定位电流 */
#define DC_HOLDTIME_TIME_LENTH         (60)               /* 单位: ms 第一段定位时间 */
#define DC_HOLDTIME_TIME_LENTH_STAGE1  (120)               /* 单位: ms 第二段定位时间 */
...
#define CLOSE_FREQ                     (300.0)              /* 单位：Hz 切入功率环的频率 */
...
#define TORQUE_MODE_CURRENT_CHANGE_ACC (0.005)              /* 单位：A/ms 电流加速调整值 */
...
#define SPEED_SET                      1700//1834//(1834)               /* 单位：Hz  速度给定值  1450 1834 */
#define SPEED_LOOP_CNTR                (0)                /* 单位：ms  速度环路计算周期 */

#define STATE04_WAITE_TIME             (0)              /* Unit: ms 速度变量初始化时间 */

#define P_ASR_KP                      3000 //4000//2000//(3000)             /* 速度环Kp */
#define P_ASR_KI                       1000//1500//6000//3000//(5000)              /* 速度环Ki */

#define IQMAX                          (0.58)                /* 单位:A, 速度环输出最大值 */
#define IQMIN                          (-0.4)               /* 单位:A, 速度环输出最小值 */

#define SPEED_RUN_ACC                  (4.0)              /* 单位 (1/128)Hz 速度加速调整值 */
#define SPEED_RUN_DEC                  (3.0)              /* 单位 (1/128)Hz 速度减速调整值 */

/* 刹车参数 */
#define BRAKE_IQ_EN                    (1)             /* IQ刹车 TRUE为使能, FALSE为不使能 */ 
#define BRAKE_CURRENT                  (-0.35)             /* 单位：A 刹车电流 */
#define BRAKE_SPEED                    (350)              /* 单位：Hz 刹车转速 */
#define BRAKE_P_CURRENT_KP             150 //100                /* 刹车电流环Kp */
#define BRAKE_P_CURRENT_KI             66// 44                /* 刹车电流环Ki */
...
/* 过欠压检测参数 */
#define U_DCB_OVERVOLTAGE_FAULT        (380.0)               /* 单位：V 过压检测设定值 */
#define U_DCB_OVERVOLTAGE_RECOVER      (370.0)               /* 单位：V 过压恢复设定值 */
#define U_DCB_UNDERVOLTAGE_FAULT       (230.0)               /* 单位：V 欠压检测设定值 */
#define U_DCB_UNDERVOLTAGE_RECOVER     (240.0)               /* 单位：V 欠压恢复设定值 */

/* 离水空转参数 */
#define I_PH_EMPTY_FAULT               (0.1)              /* 单位：A 空转检测电流设定值 */
#define SPEED_EMPTY_FAULT              (180.0)             /* 单位：Hz 空转检测转速设定值  */

/* 温度检测参数 */
#define TEMP_FAULT                     (12000)              /* 过温检测设定值 */
#define TEMP_RECOVER                   (170)              /* 过温恢复设定值 */
#define TEMP_BREAK                     (32000)             /* NTC开路设定值 */

/* 过零检测参数 */
#define ZEROUNDER_FAULT                  (8)              /* 过零检测最小值 ms*/	
#define ZEROOVER_FAULT                   (10)              /* 过零检测最大值 ms*/
                                                                  
/* 堵转检测参数 */
#define SPEED_STALL_MAX_FAULT          (2600.0)           /* 单位：Hz 堵转检测转速最大值 */
#define SPEED_STALL_MIN_FAULT          (50.0)             /* 单位：Hz 堵转检测转速最小值 */

#define I_PH_STALL_FAULT               (0.8)              /* 单位：A 堵转检测电流设定值 */
...
/* 二次启动检测参数 */
#define START_TIME_FAULT               (400)              /* 单位：5ms 开环之后1s内还不进入闭环就重启，1s这个时间根据实际应用调整 */
...
#define PHASE_CHCEK_TIME               (400)               /* 单位：5ms 缺相检测时间*/
...
#define TEMP_FAULT_RECOVER_TIME        (300)             /* 单位：ms  过温恢复时间 */
